...
uses
  ...
  ShellApi, Vcl.Grids, Vcl.DBGrids, frameBotoesMenuPadrao, Datasnap.DBClient,
...
type
  TCA_JOGOS = class(TForm)
    ...
    cbWo: TDBCheckBox;
    gbxNaturalizado: TGroupBox;
    cdsGolsDoJogo: TClientDataSet;
    cdsGolsDoJogocodjogador: TIntegerField;
    cdsGolsDoJogocodjogo: TIntegerField;
    cdsGolsDoJogotempo: TIntegerField;
    cdsGolsDoJogofracao: TWideStringField;
    cdsGolsDoJogoperiodo: TWideStringField;
    ...
    cdsGolsDoJogoItem: TIntegerField;
    dtsGolsDoJogo: TDataSource;
    cdsGolsDoJogocodtipo: TIntegerField;
    cdsGolsDoJogonome: TStringField;
    cdsGolsDoJogotipo: TStringField;
    qryGolsTitulares: TFDQuery;
    memObservacoes: TDBMemo;
    acoes: TActionList;
    actBuscarInfoJogoInternet: TAction;
    btnInfoJogo: TBitBtn;
    ...
    procedure actBuscarInfoJogoInternetExecute(Sender: TObject);
  private
    { Private declarations }
  public
    { Public declarations }
    Acessou: boolean;
    ExcluiuGolsAgora: Boolean;
...
procedure TCA_JOGOS.actBuscarInfoJogoInternetExecute(Sender: TObject);
var
  consultaJogo: String;
begin
  if (EdtMandante.Text <> EmptyStr) and (EdtVisitante.Text <> EmptyStr) and (EdtPlacarMandante.Text <> EmptyStr) and ((EdtPlacarVisitante.Text <> EmptyStr)) then
  begin
    consultaJogo := 'https://www.google.com.br/search?q=' + EdtData.Text +
      ' ' + EdtMandante.Text + ' ' + EdtPlacarMandante.Text + ' x ' + EdtPlacarVisitante.Text + ' ' + EdtVisitante.Text + ' ' + EdtCompeticao.Text +
      '&source=lmns&bih=937&biw=1920&hl=pt-BR&sa=X&ved=2ahUKEwjE4bTFor3wAhVsqZUCHUsHBwIQ_AUoAHoECAEQAA';

    ShellExecute(Handle, 'open', PChar(consultaJogo), '', '', 1);
    EdtCodigo.Hint := 'Clique duplo neste espaço para buscar informações do jogo na internet';
    EdtCodigo.ShowHint := True;
    EdtCodigo.DragCursor := crHandPoint;
  end
  else
  begin
    EdtCodigo.ShowHint := False;
    EdtCodigo.DragCursor := crDefault;
  end;
end;
...
begin
  try
    EdtData.Text := f_gerais.retornaData(EdtData.Text);
    erro := false;
  except
    erro := true;
  end;
                ...
                begin
                  ...
                  begin
                    ...
                    begin // finalmente postar os dados do jogo
                      ...
                      { LIMPAR DADOS PARA PREPARAÇÃO DA INCLUSÃO DOS GOLS }
                      if (cdsGolsDoJogo.RecordCount > 0) or ExcluiuGolsAgora then
                      begin
                         qryPesquisa := TFDQuery.Create(nil);
                         qryPesquisa.Connection := FrmDm.BDMySQL;
                         qryPesquisa.Close;
                         qryPesquisa.SQL.Clear;
                         qryPesquisa.SQL.Add(Format('DELETE FROM es_gols WHERE codjogo = %d',[codigojogo]));
                         qryPesquisa.ExecSQL;
                      end;

                      { GRAVAR GOLS DO JOGO }
                      cdsGolsDoJogo.First;
                      while not cdsGolsDoJogo.Eof do
                      begin
                          if qryPrincipal.Active then
                             qryPrincipal.Close;
                          qryPrincipal.Open;
                          qryPrincipal.Append;
                          qryPrincipalcodjogador.Value := cdsGolsDoJogocodjogador.Value;
                          qryPrincipalcodjogo.Value := cdsGolsDoJogocodjogo.Value;
                          qryPrincipaltempo.Value := cdsGolsDoJogotempo.Value;
                          qryPrincipalfracao.Value := cdsGolsDoJogofracao.Value;
                          qryPrincipalcodtipogol.Value := cdsGolsDoJogocodtipo.Value;
                          qryPrincipalperiodo.Value := cdsGolsDoJogoperiodo.Value;
                          qryPrincipal.Post;
                          cdsGolsDoJogo.Next;
                      end;

                      { GRAVAR QTDE DE GOLS DO JOGADOR NA TABELA DE ES_TITUL OU ES_RESERV }
                      //zerar gols dos titulares e reservas
                      qryPesquisa := TFDQuery.Create(nil);
                      qryPesquisa.Connection := FrmDm.BDMySQL;
                      qryPesquisa.Close;
                      qryPesquisa.SQL.Clear;
                      qryPesquisa.SQL.Add(Format('UPDATE es_titul SET gols = 0 WHERE codjogo = %d; UPDATE es_reser SET gols = 0 WHERE codjogo = %d;',[codigojogo, codigojogo]));
                      qryPesquisa.ExecSQL;

                      if qryGolsTitulares.Active then
                         qryGolsTitulares.Close;
                      qryGolsTitulares.Params.ParamByName('CodigoJogo').DataType := ftInteger;
                      qryGolsTitulares.Params.ParamByName('CodigoJogo').Value := codigojogo;
                      qryGolsTitulares.ExecSQL;
...
begin
  ...
  begin
    ...
    EdtMandante.Clear;
    EdtVisitante.Clear;
    if (f_gerais.ResultadoSQL(Format('select count(*) from es_titul where codjogo = %s',[EdtCodigo.Text])) = 0) and
       (FrmDm.DtsJogos.DataSet.State in [dsEdit])  then
       CbEscalacoDisp.Checked := True
    else
       CbEscalacoDisp.Checked := False;
  ...
  cdsGolsDoJogo.EmptyDataSet;
  ExcluiuGolsAgora := False;

  if (FrmDm.DtsJogos.DataSet.State in [dsBrowse]) and
     (f_gerais.ResultadoSQL('SELECT COUNT(*) FROM ca_jogos') = 0) then
  begin
     ImgLogoMandante.Picture.Graphic := nil;
     ImgLogoVisitante.Picture.Graphic := nil;
     ImgUniforme.Picture.Graphic := nil;
