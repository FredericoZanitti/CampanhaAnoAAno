...
uses
  ...
  DateUtils, IdHashMessageDigest, DBXJSON, Character, DBClient,
...
type
  ...
  const
    ...
    IdColunas: array[1..208] of String =
    (
  ...
  var
    ...
    procedure buscaLogoFederacaoPorCodigo(objeto: TImage; codigoFederacao: Integer);
    ...
    procedure buscaBandeiraPaisPorCodigo(objeto: TImage; codigoPais: Integer);
    ...
    procedure pesquisaFederacao(grid: TDBGrid; partenome: String);
    ...
    procedure preencherEstGosPorCompeticao(CodigoCompeticao: string; ordem: string);
    ...
    {Montra o nome das colunas no Excel}
    procedure NomeColunasExcel(Excel: Variant; Query: TFDQuery; Linha, Coluna, I: Integer); overload;
    {Montra o nome das colunas no Excel}
    procedure NomeColunasExcel(Excel: Variant; Query: TFDQuery; Linha, Coluna: Integer; Campo: string); overload;
    {Montra o nome das colunas no Excel}
    procedure NomeColunasExcel(Excel: Variant; Query: TClientDataSet; Linha, Coluna, I: Integer); overload;
    {Montra o nome das colunas no Excel}
    procedure NomeColunasExcel(Excel: Variant; Query: TClientDataSet; Linha, Coluna: Integer; Campo: string); overload;
    {Inserir os valores nas colunas do Excel}
    procedure ValoresColunasExcel(Excel: Variant; Query: TFDQuery; Linha, Coluna, I: Integer); overload;
    {Inserir os valores nas colunas do Excel}
    procedure ValoresColunasExcel(Excel: Variant; Query: TFDQuery; Linha, Coluna: Integer; Campo: string); overload;
    {Inserir os valores nas colunas do Excel}
    procedure ValoresColunasExcel(Excel: Variant; Query: TClientDataSet; Linha, Coluna, I: Integer); overload;
    {Inserir os valores nas colunas do Excel}
    procedure ValoresColunasExcel(Excel: Variant; Query: TClientDataSet; Linha, Coluna: Integer; Campo: string); overload;

    { Salva o conteúdo de uma query em Excel no disco }
    procedure ExportarDadosExcel(Query: TFDQuery; NomePlanilha: String; var Diretorio: String); overload;
    { Salva o conteúdo de uma query em Excel em memória }
    procedure ExportarDadosExcel(Query: TFDQuery; NomePlanilha: String); overload;
    ...
    function sequencias(dti, dtf: TLabel; Param: String;
      indice, temporada: Integer): String;
    function idadeAtual(DataIni, DataFim: TDateTime; somenteAno: string): string;
    ...
    function ResultadoSQL(sql: string): Variant;
    function GerarNomeArquivoDuplicado(CaminhoArquivo: String): String;
  end;
...
uses module, pesquisaJogadoresAtivos, reservas, edtiarescalacao, jogos,
  ...
  relSumula, SysUtils, ShlObj, MSHTML;

function Tf_gerais.ResultadoSQL(sql: string): variant;
{
  =======================================================
  Buscar Imagem da bandeira do país através de seu código
  =======================================================
}
var
...
begin
     ...
     qryPesquisa.Connection := FrmDm.BDMySQL;
     qryPesquisa.Close;
     qryPesquisa.SQL.Clear;
     qryPesquisa.SQL.Add(sql);
     qryPesquisa.Open;

     if qryPesquisa.Fields[0].IsNull then
        Result := EmptyStr
     else
        Result := qryPesquisa.Fields[0].Value;
end;
...
begin
  ...
  r_artilheirosporadver.Caption := 'ARTILHEIROS POR ADVERSÁRIO';
  r_artilheirosporadver.lblIdentificacao.Caption := 'Adversário';
  r_artilheirosporadver.lblAdverCampeonato.Caption := AnsiUpperCase(ResultadoSQL(Format('select nome from ca_adver where codadver = %s',[codigoTime])));
...
procedure Tf_gerais.preencherEstGosPorCompeticao(CodigoCompeticao: string;
  ordem: string);
{
  =======================================================
  Preencher artilheiros por adversário
...
begin
  sql := ' select a.codjogador, a.abreviatura, a.nome, sum(a.j) j, sum(a.gols) g, round(sum(a.gols)/sum(a.j),2) mg ' +
         ...
         '    select e.codjogador, p.abreviatura, jg.nome, sum(r.j) j, sum(e.gols) gols, round(sum(e.gols)/sum(r.j),2) mg ' +
         '    from es_titul e ' +
         '    inner join ca_jogad jg on jg.codjogador = e.codjogador ' +
         '    inner join ca_posic p on p.codposicao = jg.codposicao ' +
         '    inner join ca_jogos j on j.codjogo = e.codjogo ' +
         '    inner join es_resum r on j.codjogo = r.codjogo ' +
         '    inner join ca_compe c on j.codcompeticao = c.codcompeticao ' +
         '    where j.codcompeticao = ' + CodigoCompeticao +
         '    group by codjogador, abreviatura, nome ' +
         '    union all ' +
         '    select e.codjogador, p.abreviatura, jg.nome, sum(r.j) j, sum(e.gols) gols, round(sum(e.gols)/sum(r.j),2) mge ' +
         '    from es_reser e ' +
  ...
  r_artilheirosporadver.DbGridArtClube.DataSource := FrmDm.DtsQrEstatisticas;
  r_artilheirosporadver.Caption := 'ARTILHEIROS POR COMPETIÇÃO';
  r_artilheirosporadver.lblIdentificacao.Caption := 'Competição';
  r_artilheirosporadver.lblAdverCampeonato.Caption := AnsiUpperCase(ResultadoSQL(Format('select nome from ca_compe where codcompeticao = %s',[CodigoCompeticao])));

  // buscar logo do seu time
  f_gerais.buscaImagemPorCodigo(r_artilheirosporadver.ImgLogoAdver, '0', '0');

  // buscar bandeiras do Pais e UF
  f_gerais.buscaBandeiras(r_artilheirosporadver.ImgBandPaisAdver,
    r_artilheirosporadver.ImgUfAdver, f_gerais.buscarNome('codcidade',
    'ca_adver', 'codadver', '0'), r_artilheirosporadver.LblCidade,
    r_artilheirosporadver.LblPais, r_artilheirosporadver.LblUf);
end;
...
begin
  sql := 'select a.codjogador, a.abreviatura, a.nome, sum(a.v+a.e+a.d) j, sum(a.gols) g, round(sum(a.gols)/sum(a.v+a.e+a.d),2) mgp, sum(a.camarelo) ca, sum(a.cvermelho) cv '
    ...
    'and j.codposicao = p.codposicao ' + 'and jg.temporada = ' +
...
begin
  ...
  begin
    ...
    buscaBandJogador(bandeira,codjogador);
  end;
...
var
  ...
  seuTime, Adversario, tempo: String;
...
begin
  ...
  begin
    contder := contder + 1;
    ultimaData := QrEstGerais.Fields[2].AsDateTime;
    QrEstGerais.Next;
  end;
  ...
  tempo := f_gerais.idadeAtual(ultimaData, now, 'N');

  if contder > contvit then
  begin
    if contder = 1 then
      r_estgerais.LblResumo.Caption := 'O ' + seuTime + ' não vence o ' +
        Adversario + ' há ' + IntToStr(contder) + ' jogo [ ou '
    else
      r_estgerais.LblResumo.Caption := 'O ' + seuTime + ' não vence o ' +
        Adversario + ' há ' + IntToStr(contder) + ' jogos [ ou ';
  end
  else
  begin
    if contvit = 1 then
      r_estgerais.LblResumo.Caption := 'O ' + seuTime + ' não perde para o ' +
        Adversario + ' há ' + IntToStr(contvit) + ' jogo [ ou '
    else
      r_estgerais.LblResumo.Caption := 'O ' + seuTime + ' não perde para o ' +
        Adversario + ' há ' + IntToStr(contvit) + ' jogos [ ou ';
  end;

  if tempo.IsEmpty then
     r_estgerais.LblResumo.Caption := r_estgerais.LblResumo.Caption + 'há algumas horas' + ' ]'
  else
     r_estgerais.LblResumo.Caption := r_estgerais.LblResumo.Caption + tempo + ' ]'
...
procedure Tf_gerais.buscaLogoFederacaoPorCodigo(objeto: TImage; codigoFederacao: Integer);
{
  =========================================================
  Buscar Imagem da logo da federação através de seu código
  =========================================================
}
var
  sql: string;
  qryLogoFederacao: TFDQuery;
begin
     sql := 'SELECT brasao FROM ca_federacao where codfederacao = ' + IntToStr(codigoFederacao);
     qryLogoFederacao := TFDQuery.Create(nil);
     qryLogoFederacao.Connection := FrmDm.BDMySQL;
     qryLogoFederacao.Close;
     qryLogoFederacao.sql.Clear;
     qryLogoFederacao.sql.Add(sql);
     qryLogoFederacao.Open;

     if FileExists(pchar(qryLogoFederacao.Fields[0].AsString)) then
       objeto.Picture.LoadFromFile(qryLogoFederacao.Fields[0].AsString)
     else
       objeto.Picture.Graphic := nil;
end;

procedure Tf_gerais.buscaBandeiraPaisPorCodigo(objeto: TImage; codigoPais: Integer);
{
  =======================================================
  Buscar Imagem da bandeira do país através de seu código
  =======================================================
...
var
  sql: string;
  qryBandeiraPais: TFDQuery;
begin
     sql := 'SELECT bandeira FROM ca_pais where codpais = ' + IntToStr(codigoPais);
     qryBandeiraPais := TFDQuery.Create(nil);
     qryBandeiraPais.Connection := FrmDm.BDMySQL;
     qryBandeiraPais.Close;
     qryBandeiraPais.sql.Clear;
     qryBandeiraPais.sql.Add(sql);
     qryBandeiraPais.Open;

     if FileExists(pchar(qryBandeiraPais.Fields[0].AsString)) then
       objeto.Picture.LoadFromFile(qryBandeiraPais.Fields[0].AsString)
     else
       objeto.Picture.Graphic := nil;
end;
...
begin
  ...
  begin
    sql := 'select ifnull(max(' + codigo + '),0) from ' + tabela;
  end;
  QrFunctions.Close;
  QrFunctions.sql.Clear;
  QrFunctions.sql.Add(sql);
...
begin
  ...
  begin
    sql := 'select e.codestadio codigo, e.nome estadio, c.nome cidade, u.sigla uf, p.nome pais '
      + 'from CA_ESTAD e, CA_CIDAD c, CA_PAIS p, CA_UF u ' +
      'where c.codcidade = e.codcidade ' + 'and c.codpais = p.codpais ' +
      'and c.coduf = u.coduf ' + 'and (e.nome like :PARTEDONOME or c.nome like :PARTEDONOME)' +
      ' order by e.nome';
  end;
...
begin
  sql := 'select j.codjogo, j.data, j.hora, am.codadver, j.placar1, ' +
    'av.codadver, j.placar2, e.nome estadio, co.nome competicao, ci.nome cidade, p.fifa siglap '
  ...
  if QrUltimo.Fields[3].AsString = EmptyStr then
  begin
      mand.Picture.Graphic := nil;
      visit.Picture.Graphic := nil;
  end
  else
  begin
...
begin
  sql := 'select a.nome time, c.nome cidade, p.fifa pais ' +
...
procedure Tf_gerais.pesquisaFederacao(grid: TDBGrid; partenome: String);
{
  =======================================================
  Pesquisar federações
  Esta procedure preenche o grid com as federações que
  correspondem ao filtro da pesquisa por parte do nome
  que é passado como parâmetro.
  =======================================================
}
var
  qryPesquisarFederacao: TFDQuery;
  dtsPesquisarFederacao: TDataSource;
begin
  if partenome = 'TODOS' then
  begin
    sql := 'select * from CA_FEDERACAO order by federacao';
  end
  else
  begin
    sql := 'select * from CA_FEDERACAO ' + 'where federacao like :PARTEDONOME '
      + 'order by federacao';
  end;

  qryPesquisarFederacao := TFDQuery.Create(nil);
  qryPesquisarFederacao.Connection := FrmDm.BDMySQL;
  dtsPesquisarFederacao := TDataSource.Create(nil);
  dtsPesquisarFederacao.DataSet := qryPesquisarFederacao;

  qryPesquisarFederacao.Close;
  qryPesquisarFederacao.sql.Clear;
  qryPesquisarFederacao.sql.Add(sql);
  if partenome <> 'TODOS' then
  begin
    qryPesquisarFederacao.Params.ParamByName('PARTEDONOME').AsString :=
      '%' + partenome + '%';
  end;
  qryPesquisarFederacao.Open;

  grid.DataSource := dtsPesquisarFederacao;
end;
...
begin
  ...
  r_jogospadrao.QrExcel.Params.ParamByName('LIMITE').AsInteger := FrmDm.QrContador.Fields[0].AsInteger;
...
begin
  ...
  if (anocompeticao = 'MnJogosAno') or
     (anocompeticao = 'estatisticasporano') then
  //selecionar somente os anos em que o time jogou alguma partida
  begin
      sql := 'select distinct temporada from ca_jogos order by temporada desc ';
  end
  else
  begin
...
begin
  ...
  sql := 'select fifa, nome from ca_pais order by nome';
...
begin
  ...
  { NUMERAÇÃO DOS RELATÓRIOS / DESCRIÇÃO
    ...
    42 - Todas as estreias por campeonato
  }
  ...
  if nroRelatorio = '42' then
  // Relatório 17 - Jogos por campeonato
  begin
    sql := 'select j.codjogo, j.data, j.codadvermand, j.placar1, j.placar2, j.codadvervisit, j.codestadio, j.codcompeticao, j.publico '
      + 'from JogosEstreias j ' + 'where codcompeticao = ' +
      r_jogospadrao.codauxiliar1 + ' order by data desc ' +
      'limit :LIMITE offset :CORTE';
  end;
...
begin
  ...
  begin
    ...
    h_fichaindividual.LblDtNasc.Caption := 'Nascimento: ' +
      ResultadoSQL('select DATE_FORMAT(dtnasc,''%d/%m/%Y'') from ca_jogad where codjogador = ' + IntToStr(codjogador)) + ' ( ' +
      f_gerais.idadeAtual(StrToDateTime(ResultadoSQL('select DATE_FORMAT(dtnasc,''%d/%m/%Y'') from ca_jogad where codjogador = ' + IntToStr(codjogador))),Now, 'S') + ' ) ';
    ...
    h_fichaindividual.LblIdadeUltimoJogo.Caption :=
      ...
      QrTitulares.Fields[2].AsDateTime, 'N');
    ...
    h_fichaindividual.LblIdadeEstreia.Caption :=
      f_gerais.idadeAtual(StrToDateTime(f_gerais.buscarNome('dtnasc','ca_jogad', 'codjogador', IntToStr(codjogador))),QrTitulares.Fields[2].AsDateTime, 'N');
...
var
  i, z, m, a, x, jogoNumero: Integer;
  aux: String;
  patroc, fornec, gols, dataJogo: String;
  {
    =======================================================
    Preencher súmula
    =======================================================
...
begin
  //limpar bandeira dos reservas
  for I := 1 to 11 do
    (r_sumula.FindComponent('ImgBandJogRes' + IntToStr(i))as TImage).Picture.Graphic := nil;

  sql := 'select codcompeticao, codadvermand, placar1, penalty1, penalty2, ' +
    'placar2, codadvervisit, data, hora, codestadio, publico, codarbitro, codtatica, contra, codfase, coduniforme, portaofechado, observacao '
  ...
  dataJogo := #39 + StringReplace(Trim(FormatDateTime('yyyy"-"mm"-"dd"',QrSumula.Fields[7].AsDateTime)),#39,#39+#39,[rfReplaceAll]) + #39;
  jogoNumero := ResultadoSQL(Format(' SELECT count(*) FROM ca_jogos WHERE data <= %s ',[dataJogo]));
  ...
  r_sumula.LblCampeonato.Caption :=
    ...
    'codfase', QrSumula.Fields[14].AsString) + ' ( JOGO Nº ' + IntToStr(jogoNumero) + ' )');
  ...
  // observacoes
  if QrSumula.Fields[17].AsString = EmptyStr then
     r_sumula.lblObservacoes.Caption := EmptyStr
  else
     r_sumula.lblObservacoes.Caption := 'OBSERVAÇÃO: ' + QrSumula.Fields[17].AsString;
  ...
  begin
    ...
    begin
      ...
      // bandeira da nacionalidade do jogador
      buscaBandJogador((r_sumula.FindComponent('ImgBandJogRes' + IntToStr(i))
        as TImage), QrReservas.Fields[0].AsString);
...
begin
  ...
  begin
    ...
    begin
      if QrEstat.Eof then
      begin
        (r_aniversariantes.FindComponent('LblNome' + IntToStr(i)) as TLabel).Visible := false;
        (r_aniversariantes.FindComponent('LblPeriodo' + IntToStr(i)) as TLabel).Visible := false;
        (r_aniversariantes.FindComponent('LblIdadeJogador' + IntToStr(i)) as TLabel).Visible := false;
        (r_aniversariantes.FindComponent('ImgBandNac' + IntToStr(i)) as TImage).Visible := false;
      end
      else
      begin
        (r_aniversariantes.FindComponent('LblNome' + IntToStr(i)) as TLabel).Visible := true;
        (r_aniversariantes.FindComponent('LblPeriodo' + IntToStr(i)) as TLabel).Visible := true;
        (r_aniversariantes.FindComponent('LblIdadeJogador' + IntToStr(i)) as TLabel).Visible := true;
        (r_aniversariantes.FindComponent('ImgBandNac' + IntToStr(i)) as TImage).Visible := true;

        (r_aniversariantes.FindComponent('LblCodigo' + IntToStr(i)) as TLabel).Caption := QrEstat.Fields[0].AsString;
        (r_aniversariantes.FindComponent('LblNome' + IntToStr(i)) as TLabel).Caption := AnsiUpperCase(QrEstat.Fields[1].AsString);
        (r_aniversariantes.FindComponent('LblPeriodo' + IntToStr(i)) as TLabel).Caption := QrEstat.Fields[2].AsString;
        (r_aniversariantes.FindComponent('LblIdadeJogador' + IntToStr(i)) as TLabel).Caption := f_gerais.idadeAtual(QrEstat.Fields[2].AsDateTime, Now, 'S');
        f_gerais.buscaBandJogador((r_aniversariantes.FindComponent('ImgBandNac' + IntToStr(i)) as TImage), QrEstat.Fields[0].AsString);
      end;
      QrEstat.Next;
    end;
...
begin
  {
    ...
    14- Maiores artilheiros de penaltis
    15- Maiores artilheiros de faltas
    ======================================================================
  }
  ...
  begin
      ...
      case codRelatorio of
        ...
        14:
          begin
            r_dezmais.LblEstatistica1.Caption := 'Gols: ' + QrEstat.Fields
              [2].AsString;
            r_dezmais.LblTitulos.Caption := 'MAIORES ARTILHEIROS - PENALTIS';
          end;
        15:
          begin
            r_dezmais.LblEstatistica1.Caption := 'Gols: ' + QrEstat.Fields
              [2].AsString;
            r_dezmais.LblTitulos.Caption := 'MAIORES ARTILHEIROS - FALTAS';
          end;
      end; // fim do case
      ...
      begin
        if not QrEstat.Eof then
        begin
            (r_dezmais.FindComponent('LblCodigo' + IntToStr(i)) as TLabel).Caption := QrEstat.Fields[0].AsString;
            (r_dezmais.FindComponent('LblNome' + IntToStr(i)) as TLabel).Caption := AnsiUpperCase(QrEstat.Fields[1].AsString);
            (r_dezmais.FindComponent('LblPeriodo' + IntToStr(i)) as TLabel).Caption := periodoJogador(QrEstat.Fields[0].AsString);
            f_gerais.buscaBandJogador((r_dezmais.FindComponent('ImgBandNac' + IntToStr(i)) as TImage), QrEstat.Fields[0].AsString);

            case codRelatorio of
              1:
                begin
                  (r_dezmais.FindComponent('LblEstatistica' + IntToStr(i)) as TLabel).Caption := QrEstat.Fields[2].AsString;
                  r_dezmais.LblTitEst.Caption := 'Gols';
                end;
              2:
                begin
                  (r_dezmais.FindComponent('LblEstatistica' + IntToStr(i)) as TLabel).Caption := QrEstat.Fields[3].AsString + ' / ' + QrEstat.Fields
                    [2].AsString;
              ...
              5:
                begin
                  (r_dezmais.FindComponent('LblEstatistica' + IntToStr(i)) as TLabel).Caption := 'Total: ' + QrEstat.Fields[2].AsString + ' / ' +
                    QrEstat.Fields[3].AsString + 'A / ' + QrEstat.Fields[4].AsString + 'V';
            ...
            (r_dezmais.FindComponent('lblPos' + IntToStr(i)) as TLabel).Visible := True;
            (r_dezmais.FindComponent('ImgBandNac' + IntToStr(i)) as TImage).Visible := True;
            (r_dezmais.FindComponent('LblNome' + IntToStr(i)) as TLabel).Visible := True;
            (r_dezmais.FindComponent('LblPeriodo' + IntToStr(i)) as TLabel).Visible := True;
            (r_dezmais.FindComponent('LblEstatistica' + IntToStr(i)) as TLabel).Visible := True;

            QrEstat.Next;
        end
        else
        begin
            (r_dezmais.FindComponent('lblPos' + IntToStr(i)) as TLabel).Visible := False;
            (r_dezmais.FindComponent('ImgBandNac' + IntToStr(i)) as TImage).Visible := False;
            (r_dezmais.FindComponent('LblNome' + IntToStr(i)) as TLabel).Visible := False;
            (r_dezmais.FindComponent('LblPeriodo' + IntToStr(i)) as TLabel).Visible := False;
            (r_dezmais.FindComponent('LblEstatistica' + IntToStr(i)) as TLabel).Visible := False;
        end;
      end; // end for
      r_dezmais.ShowModal;
  end
  else
      Application.MessageBox('Ainda sem estatísticas para este relatório!', 'ATENÇÃO', MB_OK + MB_ICONINFORMATION);
end;
...
begin
  ...
  r_campanhageral.LblSequencia1.Caption := f_gerais.sequencias(r_campanhageral.LblDtIniSeq1, r_campanhageral.LblDtFimSeq1, '0', 1, temporada);
  r_campanhageral.LblSequencia2.Caption := f_gerais.sequencias(r_campanhageral.LblDtIniSeq2, r_campanhageral.LblDtFimSeq2, '0', 2, temporada);
  r_campanhageral.LblSequencia3.Caption := f_gerais.sequencias(r_campanhageral.LblDtIniSeq3, r_campanhageral.LblDtFimSeq3, '0', 3, temporada);
  r_campanhageral.LblSequencia4.Caption := f_gerais.sequencias(r_campanhageral.LblDtIniSeq4, r_campanhageral.LblDtFimSeq4, '0', 4, temporada);
...
function Tf_gerais.sequencias(dti, dtf: TLabel; Param: String;
  indice, temporada: Integer): String;
var
  dtini1, dtfim1: String;
begin
  sql := Format('CALL sp_sequencias(%d, %s, %d)',[temporada, Param, indice]);
  ...
  if QrSequencias.Fields[2].AsString = EmptyStr then
    Result := 'Não houve'
  else
  begin

    dti.Caption := QrSequencias.Fields[0].AsString;
    dtf.Caption := QrSequencias.Fields[1].AsString;

    if QrSequencias.Fields[2].AsInteger = 1 then
      Result := QrSequencias.Fields[2].AsString + ' jogo, em ' + dti.Caption
    else
      Result := QrSequencias.Fields[2].AsString + ' jogos, de ' + dti.Caption + ' à ' +
        dtf.Caption;
...
begin
  sql := Format('CALL sp_Titulos_Qtde_Anos(%s);',[f_gerais.InsereAspasSimples(tipo)]);
  ...
  begin
    ...
    descricao.Caption := descricao.Caption +
                         '| ' + qryTitulos.Fields[2].AsString + ' | ' +
                         qryTitulos.Fields[1].AsString + ' [ ' +
                         qryTitulos.Fields[3].AsString + ' ]' + #13 + #13;

    qryTitulos.Next;

  end;

  qtdeTitulos := f_gerais.ResultadoSQL(Format(' SELECT COUNT(*) FROM es_titulos ' +
                                              ' INNER JOIN ca_compe ON ca_compe.codcompeticao = es_titulos.codcompeticao ' +
                                              ' WHERE ca_compe.tipocompeticao = %s',[f_gerais.InsereAspasSimples(tipo)]));

  if tipo = 'N' then
    r_titulos.lblTitulosNacionais.Caption := 'Títulos Nacionais [ ' + IntToStr(qtdeTitulos) + ' ]'
  else
  begin
    if tipo = 'R' then
       r_titulos.lblTitulosRegionais.Caption := 'Títulos Regionais [ ' + IntToStr(qtdeTitulos) + ' ]'
    else
       r_titulos.lblTitulosInternacionais.Caption := 'Títulos Internacionais [ ' + IntToStr(qtdeTitulos) + ' ]';
  end;
end;

function Tf_gerais.idadeAtual(DataIni, DataFim: TDateTime; somenteAno: string): string;
{
  =======================================================
  IDADEATUAL = FUNÇÃO QUE RETORNA A DIFERENÇA ENTRE DATAS
  NO PADRÃO ANO, DIAS, MESES EX: 7 ANOS, 5 MESES E 2 DIAS
  IMPORTANTE: NOS PARAMETROS PASSAR A DATA MAIOR NO PRI-
  MEIRO PARÂMETRO E A DATA MENOR, NO SEGUNDO.
  =======================================================
}
begin
  Result := ResultadoSQL(Format('SELECT fn_dataem_anomesdia(%s, %s, %s)',[InsereAspasSimples(FormatDateTime('yyyy-mm-dd',DataIni)),
                                                                          InsereAspasSimples(FormatDateTime('yyyy-mm-dd',DataFim)),
                                                                          InsereAspasSimples(somenteAno)]));
end;
...
begin
  ...
  begin
    ...
    if FrmDm.QrBackup.Fields[0].AsString = DateToStr(now) then
      ...
      tempo.Caption := 'ÚLTIMO BACKUP REALIZADO: ' + FrmDm.QrBackup.Fields[0]
        .AsString + ' ( ' + f_gerais.idadeAtual(FrmDm.QrBackup.Fields[0].AsDateTime, now, 'N') + ' )';

  end;

end;

procedure Tf_gerais.NomeColunasExcel(Excel: Variant; Query: TFDQuery; Linha, Coluna, I: Integer);
begin
   // Adiciona o nome da coluna
   Excel.WorkBooks[1].Sheets[1].Cells[Linha, Coluna] := Query.Fields[I].DisplayName;
   // Posiciona o título de acordo com a posição do campo
   if Query.Fields[I].Alignment = taLeftJustify then
        Excel.WorkBooks[1].Sheets[1].Range[IdColunas[Coluna],IdColunas[Coluna]].EntireColumn.HorizontalAlignment := xlLeft
   else
   if Query.Fields[I].Alignment = taRightJustify then
        Excel.WorkBooks[1].Sheets[1].Range[IdColunas[Coluna],IdColunas[Coluna]].EntireColumn.HorizontalAlignment := xlRight
   else
        Excel.WorkBooks[1].Sheets[1].Range[IdColunas[Coluna],IdColunas[Coluna]].EntireColumn.HorizontalAlignment := xlCenter;
   // Negrita o título
   Excel.WorkBooks[1].Sheets[1].Cells[Linha, Coluna].Font.Bold := True;
   if Query.Fields[I].DataType in [ftString, ftWideString] then
        Excel.WorkBooks[1].Sheets[1].Range[IdColunas[Coluna],IdColunas[Coluna]].EntireColumn.NumberFormat := '@';
end;

procedure Tf_gerais.NomeColunasExcel(Excel: Variant; Query: TFDQuery; Linha, Coluna: Integer; Campo: string);
begin
   // Adiciona o nome da coluna
   Excel.WorkBooks[1].Sheets[1].Cells[Linha, Coluna] := Query.FieldByName(Campo).DisplayName;
   // Posiciona o título de acordo com a posição do campo
   if Query.FieldByName(Campo).Alignment = taLeftJustify then
        Excel.WorkBooks[1].Sheets[1].Range[IdColunas[Coluna],IdColunas[Coluna]].EntireColumn.HorizontalAlignment := xlLeft
   else
   if Query.FieldByName(Campo).Alignment = taRightJustify then
        Excel.WorkBooks[1].Sheets[1].Range[IdColunas[Coluna],IdColunas[Coluna]].EntireColumn.HorizontalAlignment := xlRight
   else
        Excel.WorkBooks[1].Sheets[1].Range[IdColunas[Coluna],IdColunas[Coluna]].EntireColumn.HorizontalAlignment := xlCenter;
   // Negrita o título
   Excel.WorkBooks[1].Sheets[1].Cells[Linha, Coluna].Font.Bold := True;
   if Query.FieldByName(Campo).DataType in [ftString, ftWideString] then
        Excel.WorkBooks[1].Sheets[1].Range[IdColunas[Coluna],IdColunas[Coluna]].EntireColumn.NumberFormat := '@';
end;

procedure Tf_gerais.NomeColunasExcel(Excel: Variant; Query: TClientDataSet; Linha, Coluna, I: Integer);
begin
   // Adiciona o nome da coluna
   Excel.WorkBooks[1].Sheets[1].Cells[Linha, Coluna] := Query.Fields[I].DisplayName;
   // Posiciona o título de acordo com a posição do campo
...
procedure Tf_gerais.NomeColunasExcel(Excel: Variant; Query: TClientDataSet; Linha, Coluna: Integer; Campo: string);
begin
   // Adiciona o nome da coluna
   Excel.WorkBooks[1].Sheets[1].Cells[Linha, Coluna] := Query.FieldByName(Campo).DisplayName;
   // Posiciona o título de acordo com a posição do campo
...
procedure Tf_gerais.ValoresColunasExcel(Excel: Variant; Query: TFDQuery; Linha, Coluna, I: Integer);
var
   MascaraData : string;
begin
  if Query.Fields[I].Value <> null then
  begin
        // Verifica se o campo é do tipo "Date" ou "DateTime"
        if Query.Fields[I].DataType in [ftDateTime, ftDate] then
        begin
             // Define a máscara de data e hora, se for o caso
             MascaraData := 'dd/mm/aaaa';

             if Query.Fields[I].Value <= StrToDate('01/01/1900') then
                  Excel.WorkBooks[1].Sheets[1].Cells[Linha, Coluna] := ''
             else
             begin
                  Excel.WorkBooks[1].Sheets[1].Cells[Linha, Coluna].NumberFormat := MascaraData;
                  Excel.WorkBooks[1].Sheets[1].Cells[Linha, Coluna] := Query.Fields[I].Value;
             end;
        end
        else
        if (Query.Fields[I] is TIntegerField) then
        begin
             Excel.WorkBooks[1].Sheets[1].Cells[Linha, Coluna] := Query.Fields[I].Value;
        end
        else
        // Verifica se o campo é o "ano / modelo" do veículo
        if (Length(Query.Fields[I].Value) = 5) and (Copy(Query.Fields[I].Value,3,1) = '/') then
        begin
             Excel.WorkBooks[1].Sheets[1].Cells[linha, Coluna].NumberFormat := AnsiChar('@');
             Excel.WorkBooks[1].Sheets[1].Cells[Linha, Coluna] := Query.Fields[I].AsString;
        end
        else
        // Verifica se o campo é do tipo númerico e a máscara do mesmo possui o símbolo de percentual
        if (Query.Fields[I] is TNumericField) and (AnsiPos('%', TNumericField(Query.Fields[I]).DisplayFormat) > 0) then
        begin
             Excel.WorkBooks[1].Sheets[1].Cells[Linha, Coluna].NumberFormat := '##0.00%';
             Excel.WorkBooks[1].Sheets[1].Cells[Linha, Coluna] := FormatFloat('##0.00%',Query.Fields[I].Value);
        end
        else
        // Verifica se o campo é do tipo númerico e a máscara do mesmo possui o símbolo de percentual
        if (Query.Fields[I] is TFloatField) and (AnsiPos('%', TFloatField(Query.Fields[I]).DisplayFormat) > 0) then
        begin
             Excel.WorkBooks[1].Sheets[1].Cells[Linha, Coluna].NumberFormat := '##0.00%';
             Excel.WorkBooks[1].Sheets[1].Cells[Linha, Coluna] := FormatFloat('##0.00%',Query.Fields[I].Value);
        end
        else
        // Verifica se o campo é do tipo númerico e a máscara do mesmo possui máscara de moeda
        //Binary-coded Decimal field that can be converted to Currency type without a loss of precision
        // Bruno Souza - 15/01/2020 Foi removido o type cast com verificação de tipo Currency para valor 0.001, que não estava passando na validação, e não aplicava a máscara
        if (Query.Fields[I] is TBCDField) then
        begin
             Excel.WorkBooks[1].Sheets[1].Cells[Linha, Coluna] := Query.Fields[I].Value;
             Excel.WorkBooks[1].Sheets[1].Cells[Linha, Coluna].NumberFormat := '_-R$ * #.##0,00_-;-R$ * #.##0,00_-;_-R$ * "-"??_-;_-@_-';
        end
        else
        // Verifica se o campo é do tipo númerico e a máscara do mesmo possui máscara de moeda
        if ((Query.Fields[I] is TFloatField) and (TFloatField(Query.Fields[I] as TFloatField).currency)) then
        begin
             Excel.WorkBooks[1].Sheets[1].Cells[linha, Coluna] := Query.Fields[I].Value;
             Excel.WorkBooks[1].Sheets[1].Cells[linha, Coluna].NumberFormat := '_-R$ * #.##0,00_-;-R$ * #.##0,00_-;_-R$ * "-"??_-;_-@_-';
        end
       ...
       // Verifica se o campo é do tipo númerico e a máscara do mesmo possui máscara de moeda
        if ((Query.Fields[I] is TCurrencyField) and (TCurrencyField(Query.Fields[I] as TCurrencyField).currency)) then
        begin
             Excel.WorkBooks[1].Sheets[1].Cells[linha, Coluna] := Query.Fields[I].Value;
             Excel.WorkBooks[1].Sheets[1].Cells[linha, Coluna].NumberFormat := '_-R$ * #.##0,00_-;-R$ * #.##0,00_-;_-R$ * "-"??_-;_-@_-';
        end
...
procedure Tf_gerais.ValoresColunasExcel(Excel: Variant; Query: TFDQuery; Linha, Coluna: Integer; Campo: string);
var
   MascaraData : string;
begin
  if Query.FieldByName(Campo).Value <> null then
  begin
        // Verifica se o campo é do tipo "Date" ou "DateTime"
        if Query.FieldByName(Campo).DataType in [ftDateTime, ftDate] then
        begin
             // Define a máscara de data e hora, se for o caso
             MascaraData := 'dd/mm/aaaa';

             if Query.FieldByName(Campo).Value <= StrToDate('01/01/1900') then
                  Excel.WorkBooks[1].Sheets[1].Cells[Linha, Coluna] := ''
             else
             begin
                  Excel.WorkBooks[1].Sheets[1].Cells[Linha, Coluna].NumberFormat := MascaraData;
                  Excel.WorkBooks[1].Sheets[1].Cells[Linha, Coluna] := Query.FieldByName(Campo).Value;
             end;
        end
        else
        if (Query.FieldByName(Campo) is TIntegerField) then
        begin
             Excel.WorkBooks[1].Sheets[1].Cells[Linha, Coluna] := Query.FieldByName(Campo).Value;
        end
        else
        // Verifica se o campo é o "ano / modelo" do veículo
        if (Length(Query.FieldByName(Campo).Value) = 5) and (Copy(Query.FieldByName(Campo).Value,3,1) = '/') then
        begin
             Excel.WorkBooks[1].Sheets[1].Cells[linha, Coluna].NumberFormat := AnsiChar('@');
             Excel.WorkBooks[1].Sheets[1].Cells[Linha, Coluna] := Query.FieldByName(Campo).AsString;
        end
        else
        // Verifica se o campo é do tipo númerico e a máscara do mesmo possui o símbolo de percentual
        if (Query.FieldByName(Campo) is TNumericField) and (AnsiPos('%', TNumericField(Query.FieldByName(Campo)).DisplayFormat) > 0) then
        begin
             Excel.WorkBooks[1].Sheets[1].Cells[Linha, Coluna].NumberFormat := '##0.00%';
             Excel.WorkBooks[1].Sheets[1].Cells[Linha, Coluna] := FormatFloat('##0.00%',Query.FieldByName(Campo).Value);
        end
        else
        // Verifica se o campo é do tipo númerico e a máscara do mesmo possui o símbolo de percentual
        if (Query.FieldByName(Campo) is TFloatField) and (AnsiPos('%', TFloatField(Query.FieldByName(Campo)).DisplayFormat) > 0) then
        begin
             Excel.WorkBooks[1].Sheets[1].Cells[Linha, Coluna].NumberFormat := '##0.00%';
             Excel.WorkBooks[1].Sheets[1].Cells[Linha, Coluna] := FormatFloat('##0.00%',Query.FieldByName(Campo).Value);
        end
        ...
        //Binary-coded Decimal field that can be converted to Currency type without a loss of precision
        // Bruno Souza - 15/01/2020 Foi removido o type cast com verificação de tipo Currency para valor 0.001, que não estava passando na validação, e não aplicava a máscara
        if (Query.FieldByName(Campo) is TBCDField) then
        begin
             Excel.WorkBooks[1].Sheets[1].Cells[Linha, Coluna] := Query.FieldByName(Campo).Value;
             Excel.WorkBooks[1].Sheets[1].Cells[Linha, Coluna].NumberFormat := '_-R$ * #.##0,00_-;-R$ * #.##0,00_-;_-R$ * "-"??_-;_-@_-';
        end
        ...
        // Verifica se o campo é do tipo númerico e a máscara do mesmo possui máscara de moeda
        if ((Query.FieldByName(Campo) is TFloatField) and (TFloatField(Query.FieldByName(Campo) as TFloatField).currency)) then
        begin
             Excel.WorkBooks[1].Sheets[1].Cells[linha, Coluna] := Query.FieldByName(Campo).Value;
             Excel.WorkBooks[1].Sheets[1].Cells[linha, Coluna].NumberFormat := '_-R$ * #.##0,00_-;-R$ * #.##0,00_-;_-R$ * "-"??_-;_-@_-';
        end
       ...
       // Verifica se o campo é do tipo númerico e a máscara do mesmo possui máscara de moeda
        if ((Query.FieldByName(Campo) is TCurrencyField) and (TCurrencyField(Query.FieldByName(Campo) as TCurrencyField).currency)) then
        begin
             Excel.WorkBooks[1].Sheets[1].Cells[linha, Coluna] := Query.FieldByName(Campo).Value;
             Excel.WorkBooks[1].Sheets[1].Cells[linha, Coluna].NumberFormat := '_-R$ * #.##0,00_-;-R$ * #.##0,00_-;_-R$ * "-"??_-;_-@_-';
        end
...
procedure Tf_gerais.ValoresColunasExcel(Excel: Variant; Query: TClientDataSet; Linha, Coluna, I: Integer);
var
   MascaraData : string;
begin
  if Query.Fields[I].Value <> null then
  begin
        ...
        // Verifica se o campo é do tipo númerico e a máscara do mesmo possui máscara de moeda
        if ((Query.Fields[I] is TBCDField) and (TBCDField(Query.Fields[I] as TBCDField).currency)) then
        begin
             Excel.WorkBooks[1].Sheets[1].Cells[Linha, Coluna] := Query.Fields[I].Value;
             Excel.WorkBooks[1].Sheets[1].Cells[Linha, Coluna].NumberFormat := '_-R$ * #.##0,00_-;-R$ * #.##0,00_-;_-R$ * "-"??_-;_-@_-';
        end
...
procedure Tf_gerais.ValoresColunasExcel(Excel: Variant; Query: TClientDataSet; Linha, Coluna: Integer; Campo: string);
var
   MascaraData : string;
begin
  if Query.FieldByName(Campo).Value <> null then
  begin
        ...
        // Verifica se o campo é do tipo númerico e a máscara do mesmo possui máscara de moeda
        if ((Query.FieldByName(Campo) is TBCDField) and (TBCDField(Query.FieldByName(Campo) as TBCDField).currency)) then
        begin
             Excel.WorkBooks[1].Sheets[1].Cells[Linha, Coluna] := Query.FieldByName(Campo).Value;
             Excel.WorkBooks[1].Sheets[1].Cells[Linha, Coluna].NumberFormat := '_-R$ * #.##0,00_-;-R$ * #.##0,00_-;_-R$ * "-"??_-;_-@_-';
        end
...
procedure Tf_gerais.ExportarDadosExcel(Query: TFDQuery; NomePlanilha: String; var Diretorio: String);
var
  Cursor: TCursor;
  Excel: Variant;
  Linha,
...
begin
  ...
  try
       ...
       // Verifica se a planilha deve ser aberta ou salva
       Salvar := Diretorio <> EmptyStr;
       // Instancia e atualiza o formulário de progressão somente ao exibir o excel
       if not(Salvar) then
       begin
            frmProgresso.gauProgresso.MaxValue := Query.RecordCount;
            frmProgresso.gauProgresso.MinValue := 0;
            frmProgresso.gauProgresso.Progress := 0;
            frmProgresso.Show;
            frmProgresso.lblMensagem.Caption := 'Gerando Planilha...';
            frmProgresso.lblMensagem.Update;
       end;
       // Posiciona no primeiro item
       Query.First;
       // Cria um objeto do Excel
       Excel := CreateOleObject('Excel.Application');
       // Define o nome da planilha
       Excel.Caption := NomePlanilha;
       // Adiciona um novo documento
       Excel.WorkBooks.Add;
       // Define os valores iniciais de linha e coluna
       Linha := 1;
       Coluna := 1;
       // Monta os títulos das colunas da planilha
       for I := 0 to Query.FieldCount - 1 do
       begin
            // Verifica se o campo está visível
            if Query.Fields[i].Visible then
            begin
                 //Montar o nome das Colunas
                 NomeColunasExcel(Excel, Query, Linha, Coluna, I);
                 Inc(Coluna);
                 // Atualiza o formulário de progressão somente quando exibe o excel
                 if not(Salvar) then
                 begin
                      frmProgresso.lblMensagem.Caption := 'Gerando cabeçalho da planilha...';
                      frmProgresso.lblMensagem.Update;
                 end;
            end;
       end;
       // Inclui os valores das colunas da planilha
       Linha := 2;
       while not Query.Eof do
       begin
            Coluna := 1;
            ...
            begin
                 ...
                 if Query.Fields[i].Visible then
                 begin
                      ValoresColunasExcel(Excel, Query, Linha, Coluna, I);
                      Inc(Coluna);
                 end;
            end;
            linha := Linha + 1;
            // Se for salvar o excel, exibe a progressão
            if not(Salvar) then
            begin
                 frmProgresso.lblMensagem.Caption := 'Gerando linha nº '+ IntToStr(linha + 1)+'...';
                 frmProgresso.lblMensagem.Update;
                 frmProgresso.gauProgresso.Progress := Linha + 1;
            end;
            Application.ProcessMessages;
            Query.Next;
       end;
       // Cria um loop para percorrer todas as colunas do arquivo
       for I := 1 to Coluna do
            // Ajusta o tamanho da coluna automaticamente
            Excel.WorkBooks[1].Sheets[1].Range[IdColunas[I], IdColunas[I]].EntireColumn.AutoFit;
       // Verifica se o Excel deve ser salvo
       if Salvar then
       begin
            ...
            begin
                 ...
                 if not SysUtils.DeleteFile(Diretorio) then
                 begin
                      // Atualiza o nome do diretório para um arquivo que ainda não existe no sistema
                      Diretorio := GerarNomeArquivoDuplicado(Diretorio);
                 end;
            end;
            // Salva o arquivo atual
            Excel.WorkBooks[1].SaveAs(Diretorio);
            // Fecha o Excel
            Excel.Quit;
       end
       else
       ...
       if Assigned(frmProgresso) then
            frmProgresso.Free;
       // Modifica o cursor para o ícone salvo no começo do procedimento
       Screen.Cursor := Cursor;
  end;
end; { ExportarDadosExcel }

procedure Tf_gerais.ExportarDadosExcel(Query: TFDQuery; NomePlanilha: String);
var
  Diretorio: String;
begin
     // Limpa o valor do diretório para que a planilha seja exibida
     Diretorio := EmptyStr;
     // Gera a planilha
     ExportarDadosExcel(Query, NomePlanilha, Diretorio);
end; { ExportarDadosExcel }

function Tf_gerais.GerarNomeArquivoDuplicado(CaminhoArquivo: String): String;
var
  ArquivoOriginal,
  Extensao: String;
  I: Integer;
begin
  ...
  Result := ArquivoOriginal;
  I := 0;
  while FileExists(Result) do
  begin
       Inc(I);
       Result := Copy(ArquivoOriginal, 1, (Length(ArquivoOriginal) - Length(Extensao))) + ' (' + IntToStr(I) + ')' + Extensao;
  end;
end; { GerarNomeArquivoDuplicado }
