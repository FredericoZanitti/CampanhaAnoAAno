...
begin
  if habilitarBotaoJogos = EmptyStr then
     ...
     Application.MessageBox(PChar(habilitarBotaoJogos), 'ATENÇÃO', MB_OK + MB_ICONINFORMATION);
end;
...
begin
  ...
  then
    r_titulos.LblTempo.Caption := 'O ' + f_gerais.buscarNome('nome', 'ca_adver',
      ...
      (FrmDm.QrGeral.Fields[0].AsDateTime, now, 'N') + ' sem conquistar um título!'
  else
...
begin
    ...
    if f_gerais.ResultadoSQL('SELECT COUNT(*) FROM ca_jogad') = 0 then
       msg := 'É necessário haver no mínimo 11 jogadores e 1 técnico cadastrados!';

    if f_gerais.ResultadoSQL('SELECT COUNT(*) FROM ca_adver') = 0 then
       msg := 'É necessário haver no mínimo 1 adversário cadastrado!';

    if f_gerais.ResultadoSQL('SELECT COUNT(*) FROM ca_compe') = 0 then
       msg := 'É necessário haver no mínimo 1 competição cadastrada!';

    if f_gerais.ResultadoSQL('SELECT COUNT(*) FROM ca_estad') = 0 then
       msg := 'É necessário haver no mínimo 1 estádio cadastrado!';

    if f_gerais.ResultadoSQL('SELECT COUNT(*) FROM ca_arbit') = 0 then
       msg := 'É necessário haver no mínimo 1 árbitro cadastrado!';

    if f_gerais.ResultadoSQL('SELECT COUNT(*) FROM ca_tatic') = 0 then
       msg := 'É necessário haver no mínimo 1 tática cadastrada!';
