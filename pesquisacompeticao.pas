...
uses
  ...
  System.Classes, Vcl.Graphics, FireDAC.Stan.Intf,
...
uses funcoes, module, competicao, jogos, telapadraojogos, selecionarano,
  titulos, relatartilheirosporadver;
...
begin
  ...
  // ==========================================================================
  // Pesquisa para relatório todos os jogos de estreia de um campeonato
  // ==========================================================================
  if identificacao = 'MnJogosEstreias' then
  begin
    //criar temporária com todas as estreias
    sql := 'call sp_jogos_estreiasEmTemporadas_porCompeticao(:CODIGOCOMPETICAO)';

    qryJogosEstreia := TFDQuery.Create(nil);
    qryJogosEstreia.Connection := FrmDm.BDMySQL;

    qryJogosEstreia.Close;
    qryJogosEstreia.sql.Clear;
    qryJogosEstreia.sql.Add(sql);
    qryJogosEstreia.Params.ParamByName('CODIGOCOMPETICAO').AsString := codcompet;
    qryJogosEstreia.ExecSQL;

    // guardar o número do forumlário na label
    r_jogospadrao.numerorelatorio := '42';

    // contar a quantidade de registros retornados na pesquisa
    sql := 'select sum(r.j) j, sum(r.v) v, sum(r.e) e, sum(r.d) d, sum(r.gp) gp, sum(r.gc) gc, sum(r.sg) sg '
      + 'from es_resum r, JogosEstreias j ' + 'where r.codjogo = j.codjogo ' +
      'and j.codcompeticao = ' + codcompet;
    ...
    begin
      ...
      // arpesentados na tela padrão
      sql := 'select j.codjogo, j.data, j.codadvermand, j.placar1, j.placar2, j.codadvervisit, j.codestadio, j.codcompeticao, j.publico '
        + 'from JogosEstreias j ' + 'where codcompeticao = ' + codcompet +
        ' order by data desc ' + 'limit :LIMITE offset :CORTE';
      ...
      r_jogospadrao.codauxiliar1 := codcompet;
      // títulos e descrição do relatório
      r_jogospadrao.LblDescricao1.Caption := 'TODAS AS ESTREIAS';
      r_jogospadrao.LblDescricao2.Caption := 'pelo(a) ' +
        AnsiUpperCase(f_gerais.buscarNome('nome', 'ca_compe', 'codcompeticao',
        codcompet));
      // buscar escudo do seu time
  ...
  // ==========================================================================
  // Pesquisa quando acionada pelo Jogadores / Artilheiros por campeonato
  // ==========================================================================
  if identificacao = 'MnJogadores12' then
  begin
    r_artilheirosporadver.codtime := '0';
    r_artilheirosporadver.codcompeticao := codcompet;
    f_gerais.preencherEstGosPorCompeticao(codcompet, 'G');
