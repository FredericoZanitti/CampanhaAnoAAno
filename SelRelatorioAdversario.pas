...
uses
  ...
  frameSelecionarEstado, frameSelecionarContinente;
...
type
  TfrmSelRelatorioAdversario = class(TForm)
    ...
    fraSelUf: TfraSelUf;
    fraSelContinente: TfraSelContinente;
    ...
    procedure fraSelNacionalidadechbTodosClick(Sender: TObject);
  private
...
begin
  ...
  sql := 'SELECT ca_adver.nome as Clube, ca_cidad.nome as Cidade, ca_pais.nome as Pais, '
    ...
    'AND ((:TODOSCONTINENTES <> 0) OR (ca_pais.codcontinente = :CODIGOCONTINENTE)) ' +
  ...
  // filtra continente
  if fraSelContinente.chbTodos.Checked then
  begin
    frmRelAdversarios.qryClubes.Params.ParamByName('TODOSCONTINENTES')
      .AsInteger := 1;
    frmRelAdversarios.qryClubes.Params.ParamByName('CODIGOCONTINENTE').AsInteger := 0;
  end
  else
  begin
    frmRelAdversarios.qryClubes.Params.ParamByName('TODOSCONTINENTES')
      .AsInteger := 0;
    frmRelAdversarios.qryClubes.Params.ParamByName('CODIGOCONTINENTE').AsInteger :=
      fraSelContinente.CodigoContinente;
  end;
...
procedure TfrmSelRelatorioAdversario.fraSelNacionalidadechbTodosClick(
  Sender: TObject);
begin
