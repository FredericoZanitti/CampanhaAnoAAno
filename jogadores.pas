...
type
  TCA_JOGAD = class(TForm)
    ...
    cbxNaturalizado: TDBLookupComboBox;
    lblNacionalidade: TLabel;
    chbNaturalizado: TDBCheckBox;
    ImgBandPaisNaturalizado: TImage;
    ...
    procedure chbNaturalizadoClick(Sender: TObject);
    procedure cbxNaturalizadoClick(Sender: TObject);
  private
...
begin
  ...
  QrExcel.Close;
  QrExcel.Open;
  f_gerais.ExportarDadosExcel(QrExcel,'Jogadores do ' + f_gerais.ResultadoSQL('SELECT nome FROM ca_adver WHERE codadver = 0'));
end;
...
procedure TCA_JOGAD.cbxNaturalizadoClick(Sender: TObject);
begin
   f_gerais.buscaBandeiraPaisPorCodigo(ImgBandPaisNaturalizado,cbxNaturalizado.KeyValue);
end;
...
procedure TCA_JOGAD.chbNaturalizadoClick(Sender: TObject);
begin
  if chbNaturalizado.Checked then
  begin
     cbxNaturalizado.ReadOnly := False;
     cbxNaturalizado.Visible := True;
     lblNacionalidade.Visible := True;
     if not(cbxNaturalizado.KeyValue = null) then
        f_gerais.buscaBandeiraPaisPorCodigo(ImgBandPaisNaturalizado,cbxNaturalizado.KeyValue);
  end
  else
  begin
     cbxNaturalizado.Visible := False;
     cbxNaturalizado.ReadOnly := True;
     cbxNaturalizado.KeyValue := null;
     lblNacionalidade.Visible := False;
     ImgBandPaisNaturalizado.Picture.Graphic := nil;
  end;
end;
...
begin
  ...
  begin
    ...
    chbNaturalizado.Enabled := False;
    cbxNaturalizado.Enabled := False;
  end;

  if FrmDm.DtsJogadores.DataSet.State in [dsInsert, dsEdit] then
  begin
    //checks de situação e naturalização
    if (FrmDm.DtsJogadores.DataSet.State in [dsEdit]) and (ChkSituacao.Checked) then
       ChkSituacao.Checked := True
    else
       ChkSituacao.Checked := False;

    if (FrmDm.DtsJogadores.DataSet.State in [dsEdit]) and (chbNaturalizado.Checked) then
       chbNaturalizado.Checked := True
    else
       chbNaturalizado.Checked := False;
    ...
    chbNaturalizado.Enabled := True;
    cbxNaturalizado.Enabled := True;
  end;
  ...
  //busca imagem da bandeira do naturalizado (se houver)
  chbNaturalizadoClick(Self);
    ...
    begin
      ...
      begin
        ...
        if (FrmDm.DtsJogadores.DataSet.State in [dsEdit]) and
          ...
          (NomeArquivoAnterior <> 'C:\Arquivos de programas\Campanha Ano a Ano\Seu Time\DBAcompanhamento\Imagens\indisponivel.jpg')
        then
