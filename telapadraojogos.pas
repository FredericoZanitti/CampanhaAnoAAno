...
type
  Tr_jogospadrao = class(TForm)
    ...
    QrExceltemporada: TIntegerField;
  ...
  private
    ...
    procedure ChecarAberturaSumula(codigojogo: string);
  public
    { Public declarations }
  var
...
begin
  QrExcelmandante.Value := f_gerais.ResultadoSQL(Format('SELECT nome FROM ca_adver WHERE codadver = %d',[QrExcelcodadvermand.Value]));
  QrExcelvisitante.Value := f_gerais.ResultadoSQL(Format('SELECT nome FROM ca_adver WHERE codadver = %d',[QrExcelcodadvervisit.Value]));
  QrExcelestadio.Value := f_gerais.ResultadoSQL(Format('SELECT nome FROM ca_estad WHERE codestadio = %d',[QrExcelcodestadio.Value]));
  QrExcelcompeeticao.Value := f_gerais.ResultadoSQL(Format('SELECT nome FROM ca_compe WHERE codcompeticao = %d',[QrExcelcodcompeticao.Value]));
  QrExceltemporada.Value := f_gerais.ResultadoSQL(Format('SELECT temporada FROM ca_jogos WHERE codjogo = %d',[QrExcelcodjogo.Value]));
end;

procedure Tr_jogospadrao.ChecarAberturaSumula(codigojogo: string);
begin
  // checar se escalação está disponível
  if f_gerais.ResultadoSQL(Format('select wo from ca_jogos where codjogo = %s',[codigojogo])) = 1 then
  begin
    msg := 'Súmula não disponível, jogo vencido por WO!';
  ...
  f_gerais.preencherSumula(codigojogo);
end;
