object CA_ADVER: TCA_ADVER
  Left = 0
  Top = 0
  BorderStyle = bsToolWindow
  Caption = 'Cadastro de Clubes'
  ClientHeight = 409
  ClientWidth = 573
  Color = clWhite
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  KeyPreview = True
  Menu = MainMenu
  OldCreateOrder = False
  Position = poMainFormCenter
  OnActivate = FormActivate
  OnClose = FormClose
  OnKeyDown = FormKeyDown
  PixelsPerInch = 96
  TextHeight = 13
  object edtCodigoFederacao: TDBEdit
    Left = 457
    Top = 322
    Width = 44
    Height = 21
    DataField = 'codfederacao'
    DataSource = FrmDm.DtsAdversarios
    TabOrder = 8
    Visible = False
  end
  object EdtRival: TDBEdit
    Left = 357
    Top = 322
    Width = 29
    Height = 21
    DataField = 'RIVAL'
    DataSource = FrmDm.DtsAdversarios
    Enabled = False
    TabOrder = 4
    Visible = False
  end
  object EdtEscudoAdver: TDBEdit
    Left = 22
    Top = 322
    Width = 321
    Height = 21
    DataField = 'ESCUDO'
    DataSource = FrmDm.DtsAdversarios
    Enabled = False
    TabOrder = 5
    Visible = False
  end
  object GroupBox2: TGroupBox
    Left = 23
    Top = 13
    Width = 529
    Height = 170
    Caption = '    DADOS DO CLUBE    '
    Color = clWhite
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentBackground = False
    ParentColor = False
    ParentFont = False
    TabOrder = 0
    object Label1: TLabel
      Left = 109
      Top = 25
      Width = 84
      Height = 13
      Caption = 'C'#243'digo do time'
      FocusControl = EdtCodigo
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label2: TLabel
      Left = 109
      Top = 90
      Width = 78
      Height = 13
      Caption = 'Nome do time'
      FocusControl = EdtNomeAdver
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object ImgLogoAdver: TImage
      Left = 22
      Top = 29
      Width = 73
      Height = 73
      DragCursor = crHandPoint
      Stretch = True
      OnDblClick = ImgLogoAdverDblClick
      OnMouseEnter = ImgLogoAdverMouseEnter
      OnMouseLeave = ImgLogoAdverMouseLeave
    end
    object EdtCodigo: TDBEdit
      Left = 109
      Top = 44
      Width = 97
      Height = 41
      AutoSize = False
      BevelOuter = bvSpace
      BorderStyle = bsNone
      DataField = 'CODADVER'
      DataSource = FrmDm.DtsAdversarios
      Enabled = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clMenuHighlight
      Font.Height = -27
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 4
    end
    object EdtNomeAdver: TDBEdit
      Left = 109
      Top = 108
      Width = 394
      Height = 27
      Ctl3D = True
      DataField = 'NOME'
      DataSource = FrmDm.DtsAdversarios
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentCtl3D = False
      ParentFont = False
      TabOrder = 0
      OnEnter = EdtNomeAdverEnter
      OnExit = EdtNomeAdverExit
    end
    object BtnEscolherFoto: TBitBtn
      Left = 22
      Top = 108
      Width = 73
      Height = 45
      Hint = 'Selecionar Escudo'
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = False
      TabOrder = 3
      OnClick = BtnEscolherFotoClick
    end
    object CbMeuTime: TCheckBox
      Left = 177
      Top = 141
      Width = 97
      Height = 17
      Caption = ' Meu time'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 2
      OnClick = CbMeuTimeClick
    end
    object ChkRival: TDBCheckBox
      Left = 109
      Top = 141
      Width = 62
      Height = 17
      Caption = ' Rival'
      DataField = 'rival'
      DataSource = FrmDm.DtsAdversarios
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 1
      ValueChecked = '1'
      ValueUnchecked = '0'
    end
  end
  object EdtCidade: TDBEdit
    Left = 392
    Top = 322
    Width = 59
    Height = 21
    DataField = 'CODCIDADE'
    DataSource = FrmDm.DtsAdversarios
    Enabled = False
    TabOrder = 6
    Visible = False
  end
  inline fraCidades: TfraCidades
    Left = 23
    Top = 197
    Width = 378
    Height = 134
    TabOrder = 1
    ExplicitLeft = 23
    ExplicitTop = 197
    ExplicitWidth = 378
    ExplicitHeight = 134
    inherited GroupBox2: TGroupBox
      Top = 1
      Width = 377
      ExplicitTop = 1
      ExplicitWidth = 377
      inherited BtnLocCidade: TBitBtn
        OnClick = fraCidades1BtnLocCidadeClick
      end
    end
  end
  inline fraBotoes: TfraBotoes
    Left = 23
    Top = 343
    Width = 481
    Height = 53
    TabOrder = 2
    ExplicitLeft = 23
    ExplicitTop = 343
    inherited BtnCancelar: TBitBtn
      OnClick = fraBotoesBtnCancelarClick
    end
    inherited BtnExcluir: TBitBtn
      OnClick = fraBotoesBtnExcluirClick
    end
    inherited BtnEditar: TBitBtn
      OnClick = fraBotoesBtnEditarClick
    end
    inherited BtnIncluir: TBitBtn
      OnClick = fraBotoesBtnIncluirClick
    end
    inherited BtnUltimo: TBitBtn
      OnClick = fraBotoesBtnUltimoClick
    end
    inherited BtnProximo: TBitBtn
      OnClick = fraBotoesBtnProximoClick
    end
    inherited BtnAnterior: TBitBtn
      OnClick = fraBotoesBtnAnteriorClick
    end
    inherited BtnPrimeiro: TBitBtn
      OnClick = fraBotoesBtnPrimeiroClick
    end
    inherited BtnGravar: TBitBtn
      OnClick = fraBotoesBtnGravarClick
    end
    inherited BtnPesquisar: TBitBtn
      OnClick = fraBotoesBtnPesquisarClick
    end
  end
  object BtnRelatorio: TBitBtn
    Left = 507
    Top = 346
    Width = 45
    Height = 45
    Hint = 'Emitir relat'#243'rio de todos os clubes cadastrados'
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
    ParentShowHint = False
    ShowHint = True
    TabOrder = 3
    OnClick = BtnRelatorioClick
  end
  object GroupBox1: TGroupBox
    Left = 410
    Top = 198
    Width = 142
    Height = 133
    Caption = '  FILIA'#199#195'O  '
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 7
    object ImgFederacao: TImage
      Left = 35
      Top = 22
      Width = 73
      Height = 73
      Center = True
      Proportional = True
      Stretch = True
    end
    object lblFederacao: TLabel
      Left = 10
      Top = 106
      Width = 122
      Height = 13
      Alignment = taCenter
      AutoSize = False
      Caption = 'lblFederacao'
    end
    object dblFederacao: TDBLookupComboBox
      Left = 10
      Top = 102
      Width = 122
      Height = 21
      BevelOuter = bvNone
      DataField = 'codfederacao'
      DataSource = FrmDm.DtsAdversarios
      KeyField = 'codfederacao'
      ListField = 'federacao'
      ListSource = FrmDm.dtsFederacao
      TabOrder = 0
      OnClick = dblFederacaoClick
    end
  end
  object OpenLogo: TOpenPictureDialog
    Left = 535
    Top = 65534
  end
  object MainMenu: TMainMenu
    Left = 504
    object MnArquivo: TMenuItem
      Caption = 'Arquivo'
      object MnCadastrar: TMenuItem
        Caption = '&Inserir ( F5 )'
        OnClick = MnCadastrarClick
      end
      object MnEditar: TMenuItem
        Caption = 'E&ditar'
        OnClick = MnEditarClick
      end
      object MnExcluir: TMenuItem
        Caption = '&Excluir'
        OnClick = MnExcluirClick
      end
    end
    object MnSair: TMenuItem
      Caption = 'Sair'
      OnClick = MnSairClick
    end
  end
end
